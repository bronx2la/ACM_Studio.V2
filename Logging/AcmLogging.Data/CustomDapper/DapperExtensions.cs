...
namespace AcmLogging.Data.CustomDapper
{
    public static class DapperExtensions
    {
        ...
        private static void AddDetailsToException(Exception ex, string procName, object paramList)
        {
            ...
            if (paramList is DynamicParameters dynParams)
                foreach (var item in dynParams.ParameterNames)
                    ex.Data.Add(item, dynParams.Get<object>(item).ToString());
            else
            {
